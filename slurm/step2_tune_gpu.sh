#!/bin/bash
# ============================================================
# Step 2 — Hyperparameter tuning, GPU algorithms only.
#
# Submit with:
#   sbatch step2_tune_gpu.sh
#
# Grid: 2 GPU algorithms × 50 trials = 100 jobs → array=0-99
# ============================================================

#SBATCH --array=0-99
#SBATCH --job-name=s2_tune_gpu
#SBATCH --output=logs/slurm_outputs/s2_tune_gpu/%x_%A_%a.out
#SBATCH --error=logs/slurm_errors/s2_tune_gpu/%x_%A_%a.err
#SBATCH --nodes=1
...
echo "S2-tune-gpu | alg=${algorithm} | trial=${trial_idx} | job=${SLURM_ARRAY_TASK_ID}"
