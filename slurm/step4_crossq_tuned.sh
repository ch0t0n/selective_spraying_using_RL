#!/bin/bash
# ============================================================
# Step 4 — Main results, TUNED hyperparameters, CrossQ (GPU)
#
# Identical grid to Step 1 but passes --hyperparams_json so
# train.py loads the Optuna-tuned HPs produced by Step 3.
#
...
#SBATCH --array=0-199
#SBATCH --job-name=s4_crossq_tuned
#SBATCH --output=logs/slurm_outputs/s4_crossq_tuned/%x_%A_%a.out
#SBATCH --error=logs/slurm_errors/s4_crossq_tuned/%x_%A_%a.err
#SBATCH --nodes=1
...
echo "S4-CrossQ-tuned | alg=$algorithm | set=$set | robots=$num_robots_value | seed=$seed | job=$SLURM_ARRAY_TASK_ID"
