#!/bin/bash
# ============================================================
# Step 6 — Ablation: Observation space (CrossQ, GPU)
#
# Fixed: CrossQ, N = 3, env variation 1, 1,000,000 timesteps
# Grid:  5 obs conditions × 5 seeds = 25 jobs
#
# obs_mode:
#   0 → base          (positions+vel+cap+infection+infected coords, 5N+3M)
#   1 → full          (base + wind vector + spray history, 6N+3M+2)
#   2 → no_wind       (base + spray history, no wind estimate, 6N+3M)
#   3 → no_spray_hist (base + wind vector, no spray history, 5N+3M+2)
#   4 → pos_only      (normalized robot positions only, 2N)
#
# Index layout:
#   seed_idx = index % 5
#   cond_idx = index // 5
...
#SBATCH --array=0-24
#SBATCH --job-name=s6_ablation_obs
#SBATCH --output=logs/slurm_outputs/s6_ablation_obs/%x_%A_%a.out
#SBATCH --error=logs/slurm_errors/s6_ablation_obs/%x_%A_%a.err
#SBATCH --nodes=1
...
echo "S6-ablation-obs | obs_mode=$obs_mode | seed=$seed | job=$SLURM_ARRAY_TASK_ID"
