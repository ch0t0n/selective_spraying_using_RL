...
All other tables (main results, transfer learning, reward ablation, obs
ablation) are filled directly from the training NPZ files — no separate eval
step is required.
...
> for that experiment.  Skip straight to Step 10 once Steps 5–7 training is done.
...
> **No separate CSVs for main results, transfer learning, or obs ablations** —
> those are read from training NPZs by `analyze_results.py`.
...
Transfer-learning rows are written to `results/main_transfer_latex_rows.txt`
...
> **Step 2:** 1080 jobs = transfer learning from Step 1 Set 1 default-policy
> checkpoints to target Sets 2–10.
>
