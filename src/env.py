...
class MultiRobotEnv(gym.Env):
    ...
    def __init__(self, field_info, render_mode=None, wind_par=[0, 0], num_robots=3, render_scale=5, max_steps=1000):
        ...
        self.init_robot_positions = np.array(field_info['init_positions'][:num_robots], dtype=np.float32)
    ...
    def _get_obs(self): # get current observation
        ...
        # Observation noise
        noise = self.np_random.normal(0, self.obs_noise_std, size=state.shape)
    ...
    def reset(self, seed=None, options={}): # reset and initialize starting state
        ...
        self.robot_positions = self.robot_positions.astype(np.float32)
    ...
    def step(self, actions):
        ...
        # Stochastic wind
        wind_mag = self.wind_mag + self.np_random.normal(0, self.wind_noise_std)     # Add noise to wind magnitude
        wind_dir = self.wind_dir + self.np_random.normal(0, self.wind_dir_noise_std) # Add noise to wind direction (degrees)
        ...
        for i in range(self.num_robots):        # For each robot
            ...
            ax = ax * self.thrust_power + self.np_random.normal(0, self.action_noise_std) # Scale + noise on x-thrust
            ay = ay * self.thrust_power + self.np_random.normal(0, self.action_noise_std) # Scale + noise on y-thrust
