...
def classify_run(
    ...
    if artifacts.weights_exists and steps_verified:
        ...
        if actual_steps is not None and actual_steps >= expected_steps and not artifacts.weights_exists:
            ...
            explicit_failure = True
            failure_reason = "Run already present in failed_runs.jsonl."
...
def build_failed_run_record(status_record: StatusRecord) -> dict:
    ...
    return {
        "status": "failed",
        "detection_source": "check_status_training",
...
def print_expected_overview(
    ...
    print(f"failed_runs.jsonl         : {args.failed_jsonl_path}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check training status across Slurm outputs and training logs.")
    ...
    parser.add_argument("--logs-dir", type=Path, default=None, help="Override logs/training_default_logs.")
    ...
    parser.add_argument("--failed-jsonl-name", default="failed_runs.jsonl", help="Root-level JSONL file for failed runs.")
    parser.add_argument("--no-record-failed-jsonl", action="store_true", help="Do not append newly identified failed runs to failed_runs.jsonl.")
    ...
    args.logs_dir = (args.logs_dir or (args.repo_root / "logs" / "training_default_logs")).resolve()
