...
# Run all experiments with: sbatch slurm_scripts/transfer_all_3_robots.sh

#SBATCH --array=0-44
#SBATCH --job-name=RL4PA_t3r_James
...
load_set=1
RUN_NAME="${algorithm}_from_set${load_set}_to_set${set}_seed${seed}_transfer_3_robots_cpu"

echo "SLURM_JOB_ID=${SLURM_JOB_ID:-}"
echo "SLURM_ARRAY_TASK_ID=${SLURM_ARRAY_TASK_ID:-}"
echo "ALG=$algorithm LOAD_SET=$load_set SET=$set SEED=$seed DEVICE=cpu"
echo "RUN_NAME=$RUN_NAME"

steps=1000000
num_robots=3

python3 transfer.py       --algorithm $algorithm       --load_set $load_set       --set $set       --seed $seed       --steps $steps       --num_robots $num_robots       --run_name $RUN_NAME       --device "cpu"
