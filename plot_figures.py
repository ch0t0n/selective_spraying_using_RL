...
Figures produced (saved to figures/):
  ...
  transfer_learning_curves.png → fig:transfer    (learning curves, transfer learning)
  transfer_*_robots.png       → fig:transfer    (per-N transfer curves)
...
Usage:
  python plot_figures.py --log_root logs --results_dir logs/results \
                         --figures_dir figures
"""
...
def load_learning_curves_for_n(log_root: str, version_fragment: str,
    ...
    """
    curves = {}
    sets = set_list or range(1, n_sets + 1)
...
def plot_learning_curves(log_root: str, version_fragment: str,
                         hp_label: str, figures_dir: str, set_list=None):
    """
    ...
    if hp_label == "default": 
        title = "Learning Curves — Default Hyperparameters" 
    elif hp_label == "random":
        title = "Learning Curves — Random Hyperparameters" 
    elif hp_label == "transfer":
        title = "Learning Curves — Transfer Learning Hyperparameters" 
    
    fig.suptitle(title, fontsize=13, y=1.02)

    for ax, N in zip(axes, robot_counts):
        curves = load_learning_curves_for_n(log_root, version_fragment, N, set_list=set_list)
...
def plot_combined_learning_curves(log_root: str, figures_dir: str):
    """
    2×4 grid: row 1 = default HPs, row 2 = random/tuned HPs.
    Saved as figures/combined_learning_curves.png.
    """
    robot_counts = [2, 3, 4, 5]
    rows = [
    ...
    fig.suptitle("Learning Curves — Default (top) vs Tuned (bottom) Hyperparameters",
                 fontsize=13, y=1.01)
    ...
    plt.tight_layout()
    os.makedirs(figures_dir, exist_ok=True)
    out_path = os.path.join(figures_dir, "combined_learning_curves.png")
    plt.savefig(out_path, bbox_inches="tight")
    plt.close()
    print(f"  Saved {out_path}")
...
def main():
    ...
    if not args.skip_curves:
        ...
        plot_learning_curves(args.log_root, "main_transfer",
                             "transfer", args.figures_dir, set_list=range(2, 11))
